    ...
    <section id="product" className="landing-anchor py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {PANELS.map(({ id, icon: Icon, title, description, href, linkLabel }, idx) => (
            <InViewReveal key={id} delay={idx * 90}>
              <div
                className={[
                  'group relative flex flex-col h-full rounded-[var(--radius-xl)]',
                  'border border-[var(--color-line)] bg-surface p-7',
                  'transition-[border-color,box-shadow,transform] duration-[300ms] ease-[cubic-bezier(0.16,1,0.3,1)]',
                  'hover:border-[var(--color-line-accent)] hover:shadow-[var(--shadow-card-hover)] hover:-translate-y-0.5',
                ].join(' ')}
              >
                {/* Icon */}
                <div className="mb-5 flex h-10 w-10 items-center justify-center rounded-[var(--radius-lg)] border border-[var(--color-line)] bg-elevated transition-colors duration-[200ms] group-hover:border-[var(--color-line-accent)] group-hover:bg-accent/[0.06]">
                ...
                <div
                  aria-hidden
                  className="absolute inset-x-7 bottom-0 h-px bg-accent/0 transition-colors duration-[300ms] group-hover:bg-accent/30"
                />
              </div>
            </InViewReveal>
          ))}
